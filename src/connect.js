...
const ERR_CHROME_NOT_CONNECTED = 'Internal Chrome popup is not responding.';

const DISABLE_LOGIN_BUTTONS = window.TREZOR_DISABLE_LOGIN_BUTTONS || false;
const CHROME_URL = window.TREZOR_CHROME_URL || './chrome/wrapper.html';
const POPUP_ORIGIN = window.TREZOR_POPUP_ORIGIN || 'https://connect.trezor.io';
const POPUP_PATH = window.TREZOR_POPUP_PATH || `${POPUP_ORIGIN}/${VERSION}`;
...
const POPUP_URL = window.TREZOR_POPUP_URL || `${POPUP_PATH}/popup/popup.html`;
...
function TrezorConnect() {
   ...
   * @param {boolean} value
   */
  // manager.closeAfterFailure = false;
  ...
  const LOGIN_CSS = '<style>@import url("@connect_path@/login_buttons.css")</style>';
...
function Popup(url, origin, name, width, height) {
  ...
  const top = (screen.height - height) / 2;
  const opts =
    `width=${width},height=${height},left=${left},top=${top},menubar=no` +
    ',toolbar=no' +
    ',location=no' +
    ',personalbar=no' +
    ',status=no';
